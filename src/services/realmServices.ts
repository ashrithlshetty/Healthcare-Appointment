...
async function copyFromYesterday(targetDate: string) {
  ...
  try {
    ...
    if (previousDaySlots.length === 0) {
      ...
      toast.error('No slots found from the previous day');
      return [];
    }
...
// async function pre_Signed_Url(key: string) {
//  AWS.config.update({
//    accessKeyId: 'AKIAWVWPW7BV4VAYNCFJ',
//    secretAccessKey: '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',
//    region: 'ap-south-1',
//  });
//   const s3 = new AWS.S3();
//   const params = {
//     Bucket: 'developmentexelon',
//     Key: key,
//     Expires: 60,
//   };
...
async function uploadChats(
  ...
  try {
    ...
    console.log(result);
    const updateContactLastChat = await contactCollection.updateOne(
      { _id: new BSON.ObjectId(CID) },
      {
        $set: {
          lastChat: lastChat,
          lastSeen: new Date(),
    ...
    console.log(updateContactLastChat);
  } catch (err) {
    console.log(err);
  }
}

async function uploadFile(file: File, type: string, CID: string, DID: string) {
  const key = file.name;
  const bucketName = 'developmentexelon';
